...
public class TransactionResource {
    ...
    public List<Transaction> list(
        ...
        return transactionService.list(accountId, type, from, to);
    }
    ...
    @PUT
    @Path("/{id}")
    @Operation(summary = "Update a transaction", description = "Updates transaction fields and recalculates account balance")
    @APIResponse(responseCode = "200", description = "Transaction updated")
    @APIResponse(responseCode = "404", description = "Transaction not found")
    public Response update(@Parameter(description = "Transaction ID") @PathParam("id") Long id,
                           @Valid Transaction updated) {
        Transaction result = transactionService.updateTransaction(id, updated);
